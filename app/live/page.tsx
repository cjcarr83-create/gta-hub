    ...
    <main className="px-4 pt-6">
      ...
      {!streams || streams.length === 0 ? (
        <div className="card text-center text-frost-muted">
          <p>Nobody&apos;s live right now. Check back soon.</p>
          <Link
            href="/world"
            className="mt-3 inline-block text-sm text-neon-pink hover:underline"
          >
            Kill time in The Block while you wait →
          </Link>
        </div>
      ) : (
        <div className="space-y-4">
          <Link
            href="/world"
            className="block rounded-lg border border-neon-violet/30 bg-gradient-to-r from-neon-pink/10 to-neon-violet/10 p-3 text-sm"
          >
            Waiting on a stream? <span className="text-neon-pink">Jump into The Block →</span>
          </Link>
            ...
            <Link key={stream.id} href={`/live/${stream.id}`} className="card block">
              <div className="mb-2 flex items-center gap-2">
                <span className="flex items-center gap-1 rounded-sm bg-blood px-2 py-0.5 text-[11px] font-display uppercase text-frost">
                  <span className="h-1.5 w-1.5 rounded-full bg-frost animate-pulse" />
                  Live
                </span>
